      ...
      <div className="overflow-y-auto pr-2 space-y-8 flex-grow min-h-0">
        <div className="bg-black/20 p-4 border border-border-dark">
          <p className="text-text-light">
            Предметы — это основная часть опыта FNTD 2, используемая для прогресса, призыва и улучшения юнитов.
          </p>
        </div>

        {/* Categories */}
        <section>
          <h2 className="font-pixel text-xl text-glow-green mb-3">Валюта</h2>
        ...
        <section>
          <h2 className="font-pixel text-xl text-glow-green mb-3">Билеты</h2>
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
        ...
        <section>
          <h2 className="font-pixel text-xl text-glow-green mb-3">Еда</h2>
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
        ...
        <section>
          <h2 className="font-pixel text-xl text-glow-green mb-3">Материалы</h2>
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
        ...
        <section>
          <h2 className="font-pixel text-xl text-glow-green mb-3">Подарки</h2>
          <h3 className="font-pixel text-lg text-glow-purple mb-3 mt-4">По редкости</h3>
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
            {itemData.presentsRarity.map(item => <ItemCard key={item.name} item={item} />)}
          </div>
          <h3 className="font-pixel text-lg text-glow-purple mb-3 mt-4">Сезонные</h3>
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
            {itemData.presentsSeasonal.map(item => <ItemCard key={item.name} item={item} />)}
          </div>
          <h3 className="font-pixel text-lg text-glow-purple mb-3 mt-4">По юнитам</h3>
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
